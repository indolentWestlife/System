<template>
  <div>
    <div style="margin-top:50px;">
      <el-input v-model="studentName" placeholder="请输入获奖教师姓名" style="width:80%" clearable></el-input>
      <el-button type="primary" style="margin-left: 10px;" @click="searchStudentByName">搜索</el-button>
      <el-button type="primary" style="margin-left:10px;margin-bottom: 20px;" @click="showDialog">新增获奖教师</el-button>
    </div>
    <div>
      <el-table
...
export default {
  ...
  methods: {
    ...
    deleteStudents () {
      this.$confirm('此操作将永久删除获奖教师, 是否继续?', '提示', {
    ...
    handleDelete (row) {
      this.$confirm('此操作将永久删除该获奖教师, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
